from flask import Flask, session, redirect, Blueprint, render_template, current_app, request, make_response, jsonify
from flask_restful import Resource
from book_form import BookForm
import sqlite3, json, stripe, datetime
...
def cart():
	...
	# ~ checkout_session_id=create_checkout_session(get_cart(session['uid'] if session['uid'] else ))
		return render_template('cart.html', product_data=product_data, sum_data=sum_data, products=products, checkout_session_id=checkout_session['id'], checkout_public_key=current_app.config['STRIPE_PUBLIC_KEY'])
	else:
